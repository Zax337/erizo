...
namespace erizo {
    ...
    std::string NiceConnection::getLocalDescription() {
        gchar * sdp = nice_agent_generate_local_sdp(agent_);
        printf("MINE sdp %s\n", sdp);
        return sdp;
    }
    ...
    int NiceConnection::sendData(void* buf, int len) {
        int val = -1;
        if (iceState == READY) {
            val = nice_agent_send(agent_, stream_id_, NICE_COMPONENT_TYPE_RTP, len, reinterpret_cast<const gchar*>(buf));
        }
        return val;
    }
    ...
    void NiceConnection::init() {
     ...
     //  	nice_debug_enable( TRUE );
        ...
        if ( ! stunServer_.empty() && stunPort_ != 0){
            GValue val = { 0 }, val2 = { 0 };
        ...
        g_object_set_property(G_OBJECT( agent_ ), "controlling-mode", &val);
        // Connect the signals
        ...
        stream_id_ = nice_agent_add_stream(agent_, iceComponents_);
        ...
        if ( minPort_ != 0 && maxPort_ != 0 ){
            nice_agent_set_port_range(agent_, stream_id_, NICE_COMPONENT_TYPE_RTP, (guint)minPort_, (guint)maxPort_);
        }
        ...
        nice_agent_attach_recv(agent_, stream_id_, NICE_COMPONENT_TYPE_RTP, g_main_loop_get_context(loop_),
                cb_nice_recv, this);
    ...
    bool NiceConnection::setRemoteCandidates( std::vector<CandidateInfo> &candidates ) {
        ...
        nice_agent_set_remote_candidates(agent_, stream_id_, NICE_COMPONENT_TYPE_RTP, candList);

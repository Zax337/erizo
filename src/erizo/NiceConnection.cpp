...
namespace erizo {
    ...
    int NiceConnection::sendData(unsigned int compId, const void* buf, int len) {
        ...
        if (iceState == NICE_READY) {
            val = nice_agent_send(agent_, stream_id_, compId, len, reinterpret_cast<const gchar*>(buf));
        }
        if (val != len) {
    ...
    void NiceConnection::init() {
        ...
        if ( ! stunServ_.empty() && stunPort_ != 0){
            ...
            g_value_set_string(&val, stunServ_.c_str());
        ...
        if (iceComponents_ > 1) {
            nice_agent_attach_recv(agent_, stream_id_, NICE_COMPONENT_TYPE_RTCP, context_, cb_nice_recv, this);
        }
    ...
    bool NiceConnection::setRemoteCandidates(
        ...
        for (unsigned int compId = 1; compId <= iceComponents_; compId++) {
            ...
            nice_agent_set_remote_candidates(agent_, NICE_COMPONENT_TYPE_RTP, compId, candList);
        }
